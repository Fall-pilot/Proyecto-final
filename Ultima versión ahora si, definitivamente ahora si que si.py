...
def tabla_equipos():
    """
    Esta función crea en la base de datos MySQL la tabla 'equipos_biomedicos' si no existe. 
    Define campos como ID del equipo, nombre, tipo, clase, marca, modelo, ubicación, fecha de ingreso, 
    estado y el ID del técnico asignado, vinculándolo con la tabla 'usuarios' mediante clave foránea. 
    Se usa para inicializar la estructura de datos necesaria en el sistema de inventario de equipos biomédicos.

    Returns
    -------
    None. No retorna valor, imprime confirmación de creación o error.
...
def tabla_mantenimiento():
    '''
    Esta función crea la tabla mantenimientos en la base de datos MySQL si no existe.
    Dicha tabla se utilizará para registrar los mantenimientos que se hagan a los 
    equipos biomédicos (ya sean preventivos o correctivos). 
    '''
    conexion= conectar_db()
    cursor= conexion.cursor()
    cursor.execute("""
...
def eliminar_equipo():
    """
    Verifica que existan equipos, si los hay pregunta por el ID y evalúa su formateo, luego, 
    pregunta por confirmacióny si S, se cancela la operación, de lo contrario, se elimina el equipo
    """
    conexion = None
    cursor = None
    try:
...
def registrar_mmto(usuario_id, tipoMmto):
    '''
    Permite a un técnico registrar un nuevo mantenimiento (preventivo o correctivo)
    que ha realizado a un equipo biomédico.Es la función principal para que los técnicos 
    documenten su trabajo diario. Registra trazabilidad y cumplimiento de mantenimiento.
    '''
    conexion = conectar_db()
    cursor = conexion.cursor()
    try:
...
def Historialmmtos():
    '''
    Muestra todos los mantenimientos registrados en el sistema, incluyendo detalles 
    del equipo y del técnico que los hizo.Permite a administradores o ingenieros tener una 
    visión general del historial técnico, útil para reportes, auditorías y planificación.
    '''
    conexion = conectar_db()
    cursor = conexion.cursor()
...
def ver_mis_mantenimientos(tecnico_id):
    '''
    Permite a un técnico ver solo los mantenimientos que él ha realizado.
    Le da al técnico acceso a su historial personal, útil para seguimiento 
    de su trabajo o consultas futuras.
    '''
    conexion = conectar_db()
    cursor = conexion.cursor()
...
def n_observacion_mmtos():
    '''
    Permite agregar una nueva observación (comentario adicional) a un mantenimiento 
    ya existente.Permite documentar cambios posteriores, aclaraciones o ampliaciones 
    sin eliminar la información anterior del mantenimiento.
    '''
    conexion = conectar_db()
    cursor = conexion.cursor()
...
def subir_reporte_mongo(usuario_id,nombre_usuario):
    '''
    Permite que un técnico suba un reporte técnico completo a la base de datos 
    MongoDB después de haber realizado un mantenimiento.Esta función es clave porque conecta 
    la información registrada en MySQL con MongoDB, permitiendo subir documentos ricos en
    detalles técnicos (PDF, rutas, notas), que son más difíciles de manejar solo en SQL. 
    Se convierte así en un reporte técnico digital completo y consultable.
    '''
    conexion = conectar_db()
    cursor = conexion.cursor()
    try:
...
def agregar_entrada_bitacora(bitacora_id=None):
    """
    Esta función permite agregar una nueva entrada de evento o falla en una bitácora ya existente en la base de datos MongoDB. 
    Si no se pasa 'bitacora_id' como argumento, lo solicita al usuario. 
    Verifica que la bitácora exista y que el ID sea válido, luego solicita el ID del técnico (validando que exista y sea técnico) 
    y una descripción detallada del evento (mínimo 10 caracteres). 
    Finalmente, inserta la entrada en la bitácora, actualizando también la fecha de última actualización y el técnico responsable.
    
    Returns
    -------
    True, si se agrega correctamente la entrada o False si ocurre un error, se cancela, o no se encuentra la bitácora.
    """
    try:
        if not bitacora_id:
